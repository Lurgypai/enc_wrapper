...
int main(int argc, char** argv) {
    ...
    enc_store_add_object(&store, "obj1", enc_object_layout_joined);
    enc_store_add_object(&store, "obj2", enc_object_layout_joined);
    ...
    enc_store_grains_write(store, "obj1", key);
    enc_store_close(store, key);

    store = enc_store_open("test.store", key);
    enc_store_grains_read(store, "obj1", key);
